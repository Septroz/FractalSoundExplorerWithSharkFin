...
// Shark Fin fractal: z = Re(z)^2 - abs(Im(z)) * Im(z) + i * (2 * Re(z) * Im(z)) + c
VEC2 sharkfin(VEC2 z, VEC2 c) {
  FLOAT re = z.x;
  FLOAT im = z.y;
  FLOAT nx = re*re - abs(im)*im + c.x;
  FLOAT ny = 2.0 * re * im + c.y;
  return VEC2(nx, ny);
}
...
vec3 fractal(VEC2 z, VEC2 c) {
  ...
  switch (iType) {
    ...
    case 8: DO_LOOP(sharkfin); break; // <-- Add this line
  }
